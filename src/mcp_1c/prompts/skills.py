...
Требования:
1. Используй инструмент `metadata.get` для получения информации об объекте {obj}
2. Используй инструмент `metadata.attributes` для получения списка реквизитов
...
Используй инструмент `generate.query` для генерации финального запроса с правильным синтаксисом 1С.
После генерации используй `query.validate` для проверки корректности запроса."""
...
1. Используй `metadata.get` для получения базовой информации об объекте
2. Используй `metadata.attributes` для получения списка реквизитов
3. Используй `metadata.forms` для получения списка форм
4. Используй `metadata.templates` для получения списка макетов
5. Если это документ, используй `metadata.registers` для получения регистров движений
6. Используй `metadata.references` для получения связей с другими объектами
...
Выполни следующие шаги:
1. Используй `platform.event` для получения информации о событии {event}
2. Используй `metadata.get` для получения информации об объекте {obj}
3. Используй `metadata.attributes` для получения реквизитов объекта
4. Используй `generate.handler` для генерации кода обработчика
...
Выполни следующие шаги:
1. Используй `metadata.get` для получения информации об объекте {obj}
2. Используй `metadata.attributes` для получения реквизитов
3. Используй `metadata.templates` для получения существующих макетов
4. Если указан макет, используй `template.get` и `template.parameters` для анализа
5. Используй `generate.print` для генерации кода печатной формы
...
Выполни следующие шаги:
1. Используй `code.usages` для поиска всех использований идентификатора "{name}"
2. Используй `code.resolve` для нахождения определения
...
Выполни следующие шаги:
1. Используй `code.module` для получения кода модуля {module}
2. Используй `code.validate` для синтаксической проверки
3. Используй `code.lint` для статического анализа
...
Выполни следующие шаги:
1. Используй `code.dependencies` для получения зависимостей модуля
2. Используй `code.callgraph` для построения графа вызовов
3. Используй `code.analyze` для расширенного анализа
...
Выполни следующие шаги:
1. Используй `metadata.get` для получения информации о документе {document}
2. Используй `metadata.registers` для получения регистров движений
3. Для каждого регистра получи структуру через `metadata.get`
4. Используй `generate.movement` для генерации кода движений
...
Выполни следующие шаги:
1. Используй `code.module` для получения текущего кода
2. Используй `code.format` для форматирования кода
...
Выполни следующие шаги:
1. Используй `code.module` для получения кода модуля {module}
2. {"Используй `code.procedure` для получения кода процедуры " + procedure if procedure else "Проанализируй весь модуль"}
3. Используй `code.analyze` для получения структуры кода
4. Используй `code.complexity` для оценки сложности
